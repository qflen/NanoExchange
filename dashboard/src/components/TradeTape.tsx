...
export function TradeTape() {
  ...
  return (
    <div className="flex flex-col h-full bg-panel-bg border border-panel-border rounded">
      <div className="flex items-center justify-end gap-2 px-3 py-1.5 border-b border-panel-border text-xs text-neutral-fg/60">
      ...
      <div className="grid grid-cols-4 gap-3 px-3 py-1 text-[11px] text-neutral-fg/50 border-b border-panel-border">
...
function TradeRow({ trade }: { trade: TradeMsg }) {
  ...
  return (
    <div className="grid grid-cols-4 gap-3 px-3 py-0.5 text-[11px] border-b border-panel-border/40">
      <div className="text-neutral-fg/70">{hhmmss}</div>
      <div className="text-right">{trade.quantity}</div>
      <div className={`text-right ${dir}`}>{trade.price.toFixed(2)}</div>
